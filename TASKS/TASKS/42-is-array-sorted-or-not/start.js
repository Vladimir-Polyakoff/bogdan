...
const arraySortInfo = ((inputArray) => {
    if (inputArray.forEach((element) => typeof element !== 'number')) {
        return "Некоторые элементы не являются числами"
    }
    
    if
    (inputArray.every((element, index) =>
        index > 0 ? element >= inputArray[index -1] : true // проверка сортировки по возрастанию
    )
    ) {
    ...
    if (
        inputArray.every((element, index) =>
        index > 0 ? element <= inputArray[index - 1] : true // отсартирован по убыванию
        )
    ) {
